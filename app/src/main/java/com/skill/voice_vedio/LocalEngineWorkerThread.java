...
public class LocalEngineWorkerThread extends Thread {
    private final static Logger log = LoggerFactory.getLogger(LocalEngineWorkerThread.class);
    ...
    private static final class WorkerThreadHandler extends Handler {
        ...
        private LocalEngineWorkerThread mLocalEngineWorkerThread;

        WorkerThreadHandler(LocalEngineWorkerThread thread) {
            this.mLocalEngineWorkerThread = thread;
        }

        public void release() {
            mLocalEngineWorkerThread = null;
        }

        @Override
        public void handleMessage(Message msg) {
            if (this.mLocalEngineWorkerThread == null) {
            ...
            switch (msg.what) {
                case ACTION_WORKER_THREAD_QUIT:
                    mLocalEngineWorkerThread.exit();
                    break;
                case ACTION_WORKER_JOIN_CHANNEL:
                    String[] data = (String[]) msg.obj;
                    mLocalEngineWorkerThread.joinChannel(data[0], msg.arg1);
                    break;
                case ACTION_WORKER_LEAVE_CHANNEL:
                    String channel = (String) msg.obj;
                    mLocalEngineWorkerThread.leaveChannel(channel);
                    break;
                case ACTION_WORKER_CONFIG_ENGINE:
                    Object[] configData = (Object[]) msg.obj;
                    mLocalEngineWorkerThread.configEngine((int) configData[0], (String) configData[1], (String) configData[2]);
                    break;
                case ACTION_WORKER_PREVIEW:
                    Object[] previewData = (Object[]) msg.obj;
                    mLocalEngineWorkerThread.preview((boolean) previewData[0], (SurfaceView) previewData[1], (int) previewData[2]);
                    break;
            }
        }
    }
    ...
    public final void waitForReady() {
        while (!mReady) {
            ...
            log.debug("wait for " + LocalEngineWorkerThread.class.getSimpleName());
        }
    }
...
//    }
    ...
    private LocalEngineConfig mEngineConfig;

    public final LocalEngineConfig getEngineConfig() {
        return mEngineConfig;
    }

    private final ServerEngineEventHandler mEngineEventHandler;

    public final void configEngine(int vProfile, String encryptionKey, String encryptionMode) {

        log.debug("--------------begin to create rtEngine:" + "key:" + encryptionKey + " model:" + encryptionMode + " profile:" + vProfile);
    ...
    private RtcEngine ensureRtcEngineCreatedOK() {
        if (mRtcEngine == null) {
            ...
            //配置Engine for vedio
            mRtcEngine.setEncryptionMode(mEngineConfig.encryptionMode);
            mRtcEngine.setEncryptionSecret(mEngineConfig.mEncryptionKey);
            mRtcEngine.setVideoProfile(mEngineConfig.mVideoProfile, false);

            log.debug("engine config as vedio key and model and profile");
    ...
    public ServerEngineEventHandler eventHandler() {
        return mEngineEventHandler;
    }

    public RtcEngine getRtcEngine() {

        return ensureRtcEngineCreatedOK();
    }
    ...
    public LocalEngineWorkerThread(Context context) {
        this.mContext = context;
        this.mEngineConfig = new LocalEngineConfig();
        this.mEngineEventHandler = new ServerEngineEventHandler(mContext, this.mEngineConfig);
    }
}